function out = my_spmbatch_reslice(P,P11,flags)
...
if isempty(P)
    ...
    out = reslice_images(P,P11,flags);
end
...
%==========================================================================
function refuncdat=reslice_images(P,P11,flags)
...
if flags.mask || flags.mean
    ...
    for x3 = 1:P(1).dim(3)
        ...
        for i = 1:numel(P)
            tmp = tmp + getmask(inv(P11.mat\P(i).mat),x1,x2,x3,P(i).dim(1:3),flags.wrap);
        end
...
refuncdat = zeros(P(1).dim(1),P(1).dim(2),P(1).dim(3),numel(P));
for i = 1:numel(P)
    ...
    if read_vol
        if ~isfinite(flags.interp)
            v = abs(kspace3d(spm_bsplinc(P(i),[0 0 0 ; 0 0 0]'),P11.mat\P(i).mat));
            for x3 = 1:P(1).dim(3)
                if flags.mean
                    Integral(:,:,x3) = ...
                        Integral(:,:,x3) + ...
                        nan2zero(v(:,:,x3) .* ...
                        getmask(inv(P11.mat\P(i).mat),x1,x2,x3,P(i).dim(1:3),flags.wrap));
                end
                if flags.mask
            ...
            if flags.interp>=0
                ...
                v = zeros(P(1).dim);
                for x3 = 1:P(1).dim(3)
                    [tmp,y1,y2,y3] = getmask(inv(P11.mat\P(i).mat),x1,x2,x3,P(i).dim(1:3),flags.wrap);
                ...
                for u=U'
                    g0     = double(f0==u);
                    for x3 = 1:P(1).dim(3)
                        [tmp,y1,y2,y3] = getmask(inv(P11.mat\P(i).mat),x1,x2,x3,P(i).dim(1:3),flags.wrap); % Should just do this once
