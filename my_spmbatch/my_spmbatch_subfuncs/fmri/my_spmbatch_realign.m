function P = my_spmbatch_realign(P,P11,flags)
...
%==========================================================================
if numel(P)==1
    P{1} = realign_series(P{1}, P11, flags);
elseif numel(P) > 1
    Ptmp = [P{1}(1), P11];
    Ptmp = realign_series(Ptmp, P11, flags);
    M  = Ptmp(1).mat / P11.mat;
    for i=1:numel(P{1})
        P{1}(i).mat = M * P{1}(i).mat;
    end
...
%--------------------------------------------------------------------------
if flags.graphics, plot_parameters(P,P11); end
...
%==========================================================================
function P = realign_series(P,P11,flags)
...
if isfield(flags,'sep') && isfinite(flags.sep)
    skip = sqrt(sum(P11.mat(1:3,1:3).^2)).^(-1)*flags.sep;
else
    skip = [1 1 1];
end
...
%--------------------------------------------------------------------------
if ~isempty(flags.PW)
    [y1,y2,y3] = coords([0 0 0  0 0 0],P11.mat,flags.PW.mat,x1,x2,x3);
...
%--------------------------------------------------------------------------
V   = smooth_vol(P11,flags.interp,flags.wrap,fwhm);
deg = [flags.interp*[1 1 1]' flags.wrap(:)];

[G,dG1,dG2,dG3] = spm_bsplins(V,x1,x2,x3,deg);
clear V
A0  = make_A(P11.mat,x1,x2,x3,dG1,dG2,dG3,wt,lkp);
...
for i=1:numel(P)
    ...
    countdown = -1;
    for iter=1:64
        [y1,y2,y3] = coords([0 0 0  0 0 0],P11.mat,P(i).mat,x1,x2,x3);
...
str = {'There is not enough overlap in the images to obtain a solution.',...
       ...
       P.fname,...
       ' ',...
       'Please check that the positioning information ',...
       'in the NIfTI headers is OK.',...
...
%==========================================================================
function plot_parameters(P,P11)
...
for i=1:numel(P)
    Params(i,:) = spm_imatrix(P(i).mat/P11.mat);
end
...
for i = 1:min([numel(P) 12])
    text(x,y,[sprintf('%-4.0f',i) 'fMRI vol' ',' num2str(P(i).n(1))],...
