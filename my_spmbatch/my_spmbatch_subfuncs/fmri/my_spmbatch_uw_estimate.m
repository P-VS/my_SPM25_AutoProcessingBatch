function ds = my_spmbatch_uw_estimate(P,P11,par)
...
%    on a grid centred on the voxel-centres, to a grid with a 
...
if length(ds.order) == 2
    mm          = sqrt(sum(P11.mat(1:3,1:3).^2)).*P11.dim(1:3);
...
%--------------------------------------------------------------------------
[ds.q, ds.p0] = make_q(P, P11, ds.fot, ds.sot, ds.exp_round);

%-Create matrix for regularisation of warps.
%--------------------------------------------------------------------------
H = make_H(P, P11, ds.order, ds.regorder);
...
for iter=1:ds.noi
    ...
    [ref,dx,dy,dz,P,ds,sf,dispP] = make_ref(ds,def,P,P11,xyz,M,msk,beta,dispP);
    AtA       = build_AtA(dx,dy,dz,Bx,By,Bz,ds,P,P11);
...
ds.beta = reshape(refit(P,P11,dispP,ds,def),prod(ds.order),nof);
ds.SS   = ssq;
...
function [q,ep] = make_q(P,P11,fot,sot,exp_round)
%
...
%_______________________________________________________________________
function H = make_H(P,P11,order,regorder)
...
%_______________________________________________________________________
function AtA = build_AtA(dx,dy,dz, Bx,By,Bz, ds, P, P11)
...
%_______________________________________________________________________
function [ref, dx,dy,dz, P, ds, sf, dispP] = make_ref(ds, def, P, P11, xyz, M, msk, beta, dispP)
...
% transformed into the space of P11, and used for all scans.
%
...
if rem
    [m_ref,D] = get_refD(ds,def,P,P11,xyz,msk);
end
Dty = cell(length(P), 1);
sf  = zeros(length(P), 1);
...
%_______________________________________________________________________
function [m_ref, D] = get_refD(ds, def, P, P11, xyz, msk)
% Get partials w.r.t. movements from first scan.

T    = P11.mat\ds.M;
txyz = xyz*T';
[m_ref,dx,dy,dz] = spm_bsplins(spm_bsplinc(P11, ds.hold), ...
                               txyz(:,1),...
...
D           = make_D(dx.*msk, dy.*msk, dz.*msk, txyz, P11.mat);
return;
%_______________________________________________________________________
...
%_______________________________________________________________________
function beta = refit(P,P11,dispP,ds,def)
