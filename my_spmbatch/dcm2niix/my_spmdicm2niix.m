function varargout = my_spmdicm2niix(src, niiFolder, fmt, outfname, useparfor)
...
if errorLog('', niiFolder) && ~no_save % remember niiFolder for later call
    more off;
    disp(['Xiangrui Li''s ' converter ' (with modifications by P. Van Schuerbeek)']);
end
...
%% user preference
pf.save_patientName = false; %getpref('dicm2nii_gui_para', 'save_patientName', false);
pf.save_json        = true; %getpref('dicm2nii_gui_para', 'save_json', true);
pf.use_parfor       = useparfor; %getpref('dicm2nii_gui_para', 'use_parfor', useparfor);
pf.use_seriesUID    = false; %getpref('dicm2nii_gui_para', 'use_seriesUID', false);
pf.lefthand         = true; %getpref('dicm2nii_gui_para', 'lefthand', true);
pf.scale_16bit      = false; %getpref('dicm2nii_gui_para', 'scale_16bit', false);
pf.reorient         = true; %getpref('dicm2nii_gui_para', 'reorient', true);
pf.dicom_ext        = false; %getpref('dicm2nii_gui_para', 'dicom_ext', false);
...
if contains(upper(vendortag.Manufacturer),'PHILIPS'), isvendor='Philips'; end
if contains(upper(vendortag.Manufacturer),'CANON'), isvendor='CANON'; end
if contains(upper(vendortag.Manufacturer),'UIH'), isvendor='UIH'; end
if ~exist('isvendor','var'), isvendor=''; end
...
flds = {'Columns' 'Rows' 'BitsAllocated' 'SeriesInstanceUID' 'SeriesNumber' ...
    ...
    'MRImageGradientOrientationNumber' 'MRImageLabelType' 'SliceNumberMR' 'PhaseNumber' 'PulseSequenceName'};
...
for i = 1:nRun
    s = h{i}{1};
    sN = sNs(i);
    a = [outfname hSuf{i}]; %[ProtocolName(s) hSuf{i}];
...
for i = 1:nRun
    ...
    h{i}{1}.ConversionSoftware = [converter ' version my_spmbatch'];
...
MBF = tryGetField(s, 'MultibandParameters');
if ~isempty(MBF)
    MBF_split = split(MBF,'\');
    s.MultibandAccelerationFactor = str2num(MBF_split{1});
end

Rfactor = tryGetField(s, 'AssetRFactors');
if ~isempty(Rfactor)
    s.ParallelReductionFactorInPlane = 1/Rfactor(1);
end
...
if isfield(s,'PulseSequenceName')
    if contains(s.PulseSequenceName,'ASL') && ~contains(s.PulseSequenceName,'ASLBOLD')
        if isfield(s,'InversionTime')
            s.PostLabelDelay = s.InversionTime;
            s=rmfield(s,'InversionTime');
        end
        if isfield(s,'ImageDimensionX')
            s.NumberOfPointsPerArm = s.ImageDimensionX;
            s=rmfield(s,'ImageDimensionX');
        end
        if isfield(s,'ImageDimensionY')
            s.NumberOfArms = s.ImageDimensionY;
            s=rmfield(s,'ImageDimensionY');
        end
    end
end
...
flds = { % store for nii.ext and json
    'ConversionSoftware',...
    'Modality','Manufacturer','ManufacturerModelName','ImplementationVersionName','SoftwareVersion',...
    'MagneticFieldStrength','ImagingFrequency','ImagedNucleus','ReceiveCoilName',...
    'InstitutionName','BodyPartExamined','PatientPosition',...
    'SeriesDescription','PulseSequence','PulseSequenceName','ScanningSequence','SequenceVariant','ScanOptions','SequenceName',...
    'SeriesNumber','AcquisitionNumber',...
    'ImageType','MRAcquisitionType',...
    'bval','bvec','VolumeTiming',...
    'FlipAngle','InversionTime','RepetitionTime','DelayTimeInTR','EchoTime','EchoTimes',...
    'EchoNumber','SecondEchoTime','deltaTE','EffectiveEPIEchoSpacing',...
    'PostLabelDelay','ArterialSpinLabelingType','LabelingDuration',...
    'NumberOfAverages','NumberOfExcitations','EchoTrainLength','ReadoutSeconds',...
    'PartialFourier','NumberOfPhaseEncodingSteps','PercentSampling','PercentPhaseFieldOfView',...
    'SpatialResolution','AcquisitionMatrix','PixelBandwidth',...
    'ParallelAcquisitionTechnique','ParallelReductionFactorInPlane',...
    'MultibandAccelerationFactor','CompressedSensingParameters','ReconDLStrength' ...
    'SliceTiming','SliceThickness','SliceGap',...
    'UnwarpDirection','LabelingDuration','PostLabelDelay','NumberOfPointsPerArm','NumberOfArms'};
...
s = h{1};
TR = tryGetField(s, 'RepetitionTime')/1000; % in ms
...
MBF = tryGetField(s, 'MultibandParameters');
if isempty(MBF), tryGetField(s, 'MultibandAccelerationFactor'); end
if isempty(MBF), MBF = "1"; end
MBF_split = split(MBF,'\');
MB_factor = str2num(MBF_split{1});
...
h{1}.SliceTiming = t; %0.5 - t/TR; % as for FSL custom timing
hdr.slice_code = sc;
...
for i = 1:numel(flds)
    ...
    elseif strcmp(nam, 'SliceTiming')
        if max(val)>10, val = val / 1000; end
