function [s, info, dict] = my_spmbatch_dicm_hdr(fname, dict, iFrames)
...
% 
% The optional 2nd input can be a dicom dictionary returned by my_spmbatch_dicm_dict. It may
...
% convert into nifti.
% 
% See also my_spmbatch_dicm_dict, DICM2NII, DICM_IMG, RENAME_DICM, SORT_DICM, ANONYMIZE_DICM
...
if nargin<2 || isempty(dict)
    if isempty(dict_full), dict_full = my_spmbatch_dicm_dict; end
    p.fullHdr = true;
    p.dict = dict_full; 
...
elseif ischar(dict) || iscellstr(dict) || ...
    ...
    p.fullHdr = false;
    p.dict = my_spmbatch_dicm_dict('', dict);
else
...
if isfield(s, 'ProtocolDataBlock') % GE
    ...
    function [p, dict] = updateVendor(p, vendor)
        ...
        dict_full = my_spmbatch_dicm_dict(vendor);
        if ~p.fullHdr && isfield(p.dict.Properties.UserData, 'fields')
            dict = my_spmbatch_dicm_dict(vendor, p.dict.Properties.UserData.fields);
        else
            dict = dict_full;
        end
        p.dict = dict;
...
flds = fieldnames(s1);
dict = my_spmbatch_dicm_dict(s.Manufacturer, flds);
len16 = 'AE AS AT CS DA DS DT FD FL IS LO LT PN SH SL SS ST TM UI UL US';
